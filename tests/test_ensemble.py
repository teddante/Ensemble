...
class TestEnsembleFunctions(unittest.TestCase):
    def setUp(self):
        ...
        self.dummy_client.chat.completions.create.side_effect = (
            lambda model, **kwargs: DummyResponse(f"Response from {model}")
        )
    ...
    def test_fetch_llm_responses(self):
        ...
        responses = asyncio.run(
            ensemble.fetch_llm_responses(self.dummy_client, prompt, models)
        )
    ...
    def test_refine_response(self):
        ...
        self.dummy_client.chat.completions.create.side_effect = (
            lambda model, **kwargs: DummyResponse(f"Refined {model}")
        )
        refined = asyncio.run(
            ensemble.refine_response(
                self.dummy_client, combined_prompt, refinement_model
            )
        )
