...
class TestEnsembleIntegration:
    ...
    async def test_successful_ensemble_workflow(self):
        ...
        with patch("ensemble.init_client") as mock_init_client, patch(
            "ensemble.load_config"
        ) as mock_load_config, patch("asyncio.to_thread") as mock_to_thread:
            ...
            assert "Machine learning" in printed_output
    ...
    async def test_partial_model_failure_workflow(self):
        ...
        def mock_create_completion(model, messages, **kwargs):
            ...
            return MockOpenAIClient(mock_responses).chat_completions_create(
...
"""
        ...
        with patch("config.Path") as mock_path, patch(
            "config.find_dotenv"
        ) as mock_find_dotenv:
            ...
            assert config["OPENROUTER_API_KEY"] == "user-api-key"
            assert config["MODELS"] == ["user-model-1", "user-model-2"]
            # Default should be used where user didn't specify
            assert config["REFINEMENT_MODEL_NAME"] == "default-refinement"
