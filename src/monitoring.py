...
class PerformanceMonitor:
    ...
    def finish_request(self, metrics: RequestMetrics, success: bool, 
                      response_length: int = 0, error_message: Optional[str] = None) -> None:
        ...
        self.logger.debug(f"Finished tracking request to {metrics.model}: "
                         f"success={success}, duration={metrics.duration:.2f}s")
    
    def finish_ensemble_operation(self, metrics: EnsembleMetrics, 
                                 refinement_success: bool, output_length: int = 0) -> None:
        ...
        self.logger.info(f"Ensemble operation completed: "
        ...
        with self._lock:
            if model:
                ...
                if stats['successful_requests'] > 0:
                    stats['average_latency'] = stats['total_duration'] / stats['successful_requests']
    ...
    def get_ensemble_stats(self, recent_count: Optional[int] = None) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            stats = {
                ...
                'average_model_success_rate': sum(success_rates) / len(success_rates) if success_rates else 0.0,
            }
    ...
    def get_health_status(self) -> Dict[str, Any]:
        ...
        if ensemble_stats.get('operation_success_rate', 0) < 0.9:  # Less than 90% success
            health_status = "degraded"
            issues.append(f"Low ensemble success rate: {ensemble_stats['operation_success_rate']:.2%}")
