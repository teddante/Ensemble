...
class RateLimiter:
    ...
    async def _wait_for_rate_limit(self, model_name: str) -> None:
        ...
        if len(request_times) >= self.config.requests_per_minute:
            ...
            if wait_time > 0:
                logger.info(
                    f"Rate limit: Waiting {wait_time:.2f}s for {model_name} (per-minute limit)"
                )
        ...
        if len(recent_requests) >= self.config.requests_per_second:
            ...
            if wait_time > 0:
                logger.info(
                    f"Rate limit: Waiting {wait_time:.2f}s for {model_name} (per-second limit)"
                )
                await asyncio.sleep(wait_time)
    ...
    async def _apply_backoff(self, model_name: str) -> None:
        ...
        if backoff_until > time.time():
            ...
            logger.info(
                f"Rate limit: Exponential backoff for {model_name}, waiting {wait_time:.2f}s"
            )
            await asyncio.sleep(wait_time)
