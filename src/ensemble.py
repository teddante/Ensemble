...
def log_with_preview(logger: logging.Logger, message: str, text: str, max_length: int = 150) -> None:
...
async def fetch_single_response(client: OpenAI, prompt: str, model: str, 
                               timeout: float = DEFAULT_TIMEOUT, 
                               request_metrics: Optional[RequestMetrics] = None) -> str:
    """
    ...
    for attempt in range(DEFAULT_MAX_RETRIES):
        try:
            ...
            response_time = time.time() - start_time
            error_msg = str(e)
            logger.warning(f"Error from model {model} after {response_time:.2f} seconds (attempt {attempt + 1}): {error_msg}")
...
async def fetch_llm_responses(client: OpenAI, prompt: str, models: List[str]) -> List[str]:
    ...
    tasks = [
        asyncio.create_task(fetch_single_response(client, sanitized_prompt, model, DEFAULT_TIMEOUT, metrics)) 
...
async def main():
    ...
    try:
        ...
        logger.info(f"Working with validated prompt: {prompt[:100]}..." if len(prompt) > 100 else prompt)
        ...
        try:
            ...
            for response in llm_responses:
                if not response.startswith("Error:"):
                    ...
                    logger.info(f"Ensemble processing completed (single response) in {total_execution_time:.2f} seconds")
                    return
        ...
        total_execution_time = time.time() - start_time
        logger.info(f"=== Ensemble processing completed successfully in {total_execution_time:.2f} seconds ===")
        ...
        total_execution_time = time.time() - start_time
        logger.exception(f"Critical error in main execution after {total_execution_time:.2f} seconds: {str(ex)}")
        print(f"\nError: {str(ex)}")
